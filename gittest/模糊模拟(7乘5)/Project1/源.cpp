...
{
	int p, d, P, D;
	double DDDDout1=1.58, DDDDout2 = 1.00, DDDDin1 = 1, DDDDin2 = 1;
	while (1)
	{
	  printf_s("P=");
	  scanf_s("%d", &p);
	  printf_s("D=");
	  scanf_s("%d", &d);
	  P = DDDDout1*Fuzzy(p*DDDDin1, d*DDDDin2,0);
	  D = DDDDout2*Fuzzy(p*DDDDin1, d*DDDDin2, 1);
	  printf_s("Fuzzy_P=%d\n", P);
	  printf_s("Fuzzy_D=%d\n\n", D);
	}
	return 0;
...
float Fuzzy(float P, float D,int symbol)
{
	...
	float DFF[5] = { -50,-10,0,10,50 };
	/*输出量U语言值特征点(根据赛道类型选择不同的输出值)*/
	//float UPFF[7] = { 0,15,30,45,60,75,90 };
